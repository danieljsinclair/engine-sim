...
**File**: `src/engine_sim_bridge_stub.cpp` → `libenginesim.dylib` (4.5KB)
- Fully functional POC implementation
- Generates sine-wave audio based on throttle position
- Validates C++ bridge API works correctly
...
- `src/engine_sim_bridge_stub.cpp` - Stub implementation (WORKING)
...
### Issue 2: Stub vs. Real Engine

**Current**: Stub works (libenginesim.dylib), generates sine waves
**Problem**: Doesn't prove actual engine simulation works

**Decision Point**:
- Continue fixing main build for real engine audio?
- Or ship stub for .NET wrapper validation first?

**Recommendation**: Fix main build. User wants real engine sounds, not sine waves.
...
M src/engine_sim_bridge_stub.cpp
?? CMakeLists_stub.txt
?? build_stub/
?? libenginesim.dylib (stub output)
```
...
# Stub build (always works, for testing)
make stub
...
```bash
# Build stub
clang++ -arch x86_64 -fPIC -shared -std=c++17 \
...
### Why Stub Exists
The stub (`engine_sim_bridge_stub.cpp`) generates simple sine waves to:
- Validate C++ bridge API works
- Test .NET wrapper P/Invoke
- Test CoreAudio integration
- Provide fallback when main build is blocked

**It is NOT the final solution** - just a testing tool.
...
- **Real**: Stub compiles and works (libenginesim.dylib exists)
...
- **Hypothetical**: Full engine-sim builds (main blocker to solve)
...
2. Run `make stub` to verify environment works
3. Run the sed fixes listed under "Next Steps"
4. Try `make build`
5. Iterate on blockers until engine-sim compiles
6. Test with .NET wrapper
7. **Do NOT revert git changes without asking**
8. **Do NOT delete files under git control**
9. **Document each fix in this file**
